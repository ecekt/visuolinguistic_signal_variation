...
def eval_predict_onset(model, split_dataloader, model_type, data_length, split_name):
    ...
    corr, pvalue = spearmanr(list_bleu, list_onset_target)
    print(round(corr, 3), round(pvalue, 3))

    print(split_name, 'correlation specificity vs. predicted onset')
    corr, pvalue = spearmanr(list_bleu, list_onset_pred)
    print(round(corr, 3), round(pvalue, 3))
    ...
    for i in dict_bleu:
        list_bleu_avg.append(np.mean(dict_bleu[i]))
    ...
    corr, pvalue = spearmanr(list_bleu_avg, list_onset_target_avg)
    print(round(corr, 3), round(pvalue, 3))

    print(split_name, 'AVG correlation specificity vs. predicted onset')
    corr, pvalue = spearmanr(list_bleu_avg, list_onset_pred_avg)
    print(round(corr, 3), round(pvalue, 3))
        ...
        for im in ds:
            ...
            for ppn in ds[im]:
                ...
                dataset_param_new.append({'attention_mask': torch.tensor(attention_masks),
                                          ...
                                          'selfbleu': torch.tensor(selfbleu[im]),
