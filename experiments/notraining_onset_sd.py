...
for spl in splits:
    ...
    for img in original_data[spl]:
        ...
        target_ons = torch.tensor(original_data[spl][img]['std_ons'])
        ...
        for sc in samples_closest:
            ...
            std_ons = original_data['train'][sc_img]['std_ons']

            preds += std_ons * sc_sim
            normalizer += sc_sim
    ...
    print(spl, 'correlation std target onset vs. predicted onset')
    print(round(corr, 4), round(pvalue, 4))

    corr, pvalue = spearmanr(specs, targets)
    print(spl, 'correlation image spec bleu-2 vs. std target onset')
    print(round(corr, 4), round(pvalue, 4))
