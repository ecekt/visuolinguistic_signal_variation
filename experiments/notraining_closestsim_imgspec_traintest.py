    ...
    for spl in splits:
        ...
        for img in original_data[spl]:
            ...
            for sc in samples_closest:
                ...
                if len(img_firstsims) < 5:
                    img_firstsims.append(sc_sim)

                # closest ones always retrieved from train
                mean_ons = original_data['train'][sc_img]['imgspec']
            ...
            first_sims_imgs.append(np.mean(img_firstsims))

        print(img_count)

        # print(spl, 'k', k, 'loss sqrt total', total_loss)

        firstsim_avg_split = np.mean(first_sims_imgs)
        firstsim_avgs_split.append(firstsim_avg_split)
        print('all sims avg', str(round(firstsim_avg_split, 4)))
...
for i in range(len(p_targetpred)):
    ...
    if p_targetpred[i] < 0.05:
        ...
        if 0.3 <= abs(coefs_targetpred[i]) < 0.6:
            ...
            simsavg_sig.append(firstsim_avgs_split[i])
        ...
        if 0.3 <= abs(coefs_targetpred[i]):
            simsavg_nonsig.append(firstsim_avgs_split[i])

print('all simavg close in sig.', round(np.mean(simsavg_sig), 4))
print('all simavg close in NONsig.', round(np.mean(simsavg_nonsig), 4))
