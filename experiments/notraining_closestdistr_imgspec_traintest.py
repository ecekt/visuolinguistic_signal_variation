...
for c in range(trial_count):
    ...
    for spl in splits:
        ...
        firstsim_avgs_split.append(first_sims_imgs)
...
for ti in range(len(firstsim_avgs_split)):
    for oi in range(len(firstsim_avgs_split)):
        if (ti, oi) not in pairs_seen and ti != oi:

            ttest_count += 1

            pairs_seen.append((ti, oi))
            pairs_seen.append((oi, ti))

            tstat, pvalue = ttest_ind(firstsim_avgs_split[ti], firstsim_avgs_split[oi])
            print('t-test imgspec', str(ti), str(oi))
            print(round(tstat, 4), round(pvalue, 4))
            if pvalue <= 0.05:
                ttest_sigs += 1

print('ttest count', ttest_count)
print('ttest sig', ttest_sigs)
